...
public class Bank {
    ...
    public static void setCustomers(TreeMap<Integer, Customer> customers) {
                    ...
                    if (parts.length == 6) { // Ensure that we have the right number of fields
                        ...
                        for (Transaction transaction : Transaction.allTransactions) {
                            if (transaction.getCustomerId() == customerId || 
                               (transaction.getReceiverId() != null && transaction.getReceiverId() == customerId)) {
                                customer.getTransactionHistory().push(transaction);
                            }
                        }
